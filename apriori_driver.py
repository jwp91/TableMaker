...
hsensFunc = create_hsensFunc('./data/ChiGammaTablev3/hsens.dat')

def Lt_from_hc_GammaChi(hgoal, cgoal, xim, xiv, hInterp, cInterp, Lbounds, tbounds, 
                        norm, useStoredSolution:bool = True, path_to_hsens = None, gammaValues = None):
    """
    ...
    """
    gammaToIndex = interp1d(gammaValues, np.arange(len(gammaValues)), kind = 'linear', bounds_error = None, fill_value = 'extrapolate') # Converts gamma to an index
    ...
    #except:
    #    # hsensFunc hasn't been created yet
    #    print("Creating hsensFunc")
    #    hsensFunc = create_hsensFunc(path_to_hsens)
    #    print("Finished creating hsensFunc")
    #    gamma = (ha - hgoal)/hsensFunc(xim, xiv)         # Heat loss parameter
...
start = time.time()
result = phi_mvhc(path, Lvals, tvals, file_pattern = file_pattern, phi = ['T', 'hr', 'CO', 'OH', 'CO2'], 
                    numXim = numXim, numXiv = numXiv, parallel = True,
                    ximLfrac = ximLfrac, ximGfrac = ximGfrac)
end = time.time()
print(f"Time elapsed creating T_func: {int(end - start)} seconds")

# Unpack results
funcs, tableInfo = result
T_func, hr_func, CO_func, OH_func, CO2_func = funcs
...
# Create auxiliary interpolators (used in plotting and verifying solver results)
hInterp = create_interpolator_mvlt(h_table, h_indices)
cInterp = create_interpolator_mvlt(c_table, c_indices)
TInterp = create_interpolator_mvlt(T_table, T_indices)

print("Confirm functions were generated correctly:")
print(T_func(0.06, 0.005, -1e6, 0.15, solver = 'gammaChi', path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues))
print(hr_func(0.06, 0.005, -1e6, 0.15, solver = 'gammaChi', path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues))
print(CO_func(0.06, 0.005, -1e6, 0.15, solver = 'gammaChi', path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues))
print(OH_func(0.06, 0.005, -1e6, 0.15, solver = 'gammaChi', path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues))
print(CO2_func(0.06, 0.005, -1e6, 0.15, solver = 'gammaChi', path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues))
                ...
                if np.abs(phiData[radInd][timeInd] - minPhi) < testThreshold:
                    ...
                    if radInd == 0 and timeInd == 0:
                        phiQueried[radInd][timeInd] = table(xim, xiv, h, c, useStoredSolution = False, solver = 'gammaChi', 
                                                              path_to_hsens = './data/ChiGammaTablev3/hsens.dat', gammaValues = gammaValues)
                    else:
...
#caseName = 'tjet_Le1B'
    ...
    savePath = r'./figures/ODT_aPriori/Publication/'+caseName+'/higherRes/'
    # Check if the folder exists, and create it if it doesn't
