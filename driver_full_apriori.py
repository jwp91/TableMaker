...
if reQuery:
    ...
    print("Confirm functions work correctly:")
...
if makeFigs:
    # Functions for plotting side-by-side and slices plot
    def sidebyside(odtdata, queried, ODT_xs, ODT_ts, phi = "phi", 
                   units='units', savePath = None):
        # Get case name from savePath
        case = savePath.split('/')[-2]
        print(f"Creating {phi} side-by-side plot")
        ...
        contour2 = ax.contourf(xx, tt, queried.T, 100, cmap = 'inferno', levels = levels)
    ...
    def slices(odtdata, queried, ODT_xs, ODT_ts, lines = [0.01, 0.2, 0.4, 0.6], 
        ...
        # Get case name from savePath
        case = savePath.split('/')[-2]
        print(f"Creating {phi} slices plot")
        ...
        contour1 = axs['A'].contourf(xx, tt, odtdata.T, 100, cmap = 'inferno', vmin=np.min(odtdata))
        levels = contour1.levels
        ...
        for i, line in enumerate(lines):
            ...
            ax.plot(ODT_xs, odtdata.T[timeInd], '-', color = 'b', label= 'ODT')
    ...
    for case in cases:
        print(f"########## Making figures for {case}... ##########")
        savePath = path + case + '/'
        # Check if the folder exists, and create it if it doesn't
        if not os.path.exists(savePath):
            os.makedirs(savePath)
        ...
        phis = ['T', 'CO', 'OH', 'CO2', 'hr']
        units = ['K', 'kg/m^3', 'kg/m^3', 'kg/m^3', 'W/m^3']
        ODT_xs = np.loadtxt(savePath + 'xVals.txt')
        ODT_ts = np.loadtxt(savePath + 'tVals.txt')
        xx,tt = np.meshgrid(ODT_xs,ODT_ts[:39])

        for i, phi in enumerate(phis): 
            ODTvals = np.loadtxt(savePath + f'{phi}_ODT.txt')
            queried = np.loadtxt(savePath + f'{phi}queried.txt')
            if phi == 'T':
                phi = 'Temperature' # Used only for labeling in plots
            sidebyside(ODTvals, queried, ODT_xs, ODT_ts, phi=phi, 
                       units=units[i], savePath=savePath)
            slices(ODTvals, queried, ODT_xs, ODT_ts, lines=[0.01, 0.2, 0.4, 0.6], 
                   phi=phi, units=units[i], save=True, savePath=savePath)
